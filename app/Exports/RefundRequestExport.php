...
class RefundRequestExport implements FromView, ShouldAutoSize, WithStyles,WithColumnWidths ,WithHeadings, WithEvents
{
    ...
    public function styles(Worksheet $sheet) {
        ...
        return [
            ...
            'A1:J'.$this->data['refund_list']->count() + 3 => [
                'borders' => [
                    'allBorders' => [
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->getStyle('A3:J'.$this->data['refund_list']->count() + 3) // Adjust the range as per your needs
                    ->getAlignment()
                    ->setHorizontal(Alignment::HORIZONTAL_CENTER)
                    ->setVertical(Alignment::VERTICAL_CENTER);
                $event->sheet->getStyle('A2:J2') // Adjust the range as per your needs
                    ...
                    ->setVertical(Alignment::VERTICAL_CENTER);
                $event->sheet->getStyle('D4:D'.$this->data['refund_list']->count() + 3) // Adjust the range as per your needs
                    ->getAlignment()
                    ->setHorizontal(Alignment::HORIZONTAL_LEFT)
                    ->setVertical(Alignment::VERTICAL_CENTER);
